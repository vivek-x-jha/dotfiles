...
- Global cross-harness agent policy lives in `ai/AGENTS.md`, is exposed at `~/AGENTS.md` by bootstrap, and Pi/Claude Code global config links to that home-level alias; `$CODEX_HOME` may carry its own project-level `AGENTS.md` for Codex meta work.
