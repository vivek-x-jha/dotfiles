...
local methods = require 'ui.colorify.methods'
...
return function(buf, event)
	...
	if event == 'TextChangedI' then
		...
		if conf.highlight.hex then methods.hex(buf, cur_linenr, api.nvim_get_current_line()) end

		if conf.highlight.lspvars then methods.lsp_var(buf, cur_linenr) end
		return
	end
	...
	if conf.highlight.lspvars then methods.lsp_var(buf, nil, min, max) end
