...
M.get_mappings = function(mappings, tb_to_add)
	...
	for _, v in ipairs(mappings) do
		...
		if vim.tbl_contains(excluded_groups, heading) or vim.tbl_contains(excluded_groups, desc:match '%S+') or string.find(v.lhs, '<Plug>') then
			goto continue
		end
...
M.autocmds = function(buf)
	require('ui.utils').set_cleanbuf_opts('nvcheatsheet', buf)
	...
	api.nvim_create_autocmd({ 'WinResized', 'VimResized' }, {
		group = group_id,
		callback = function()
			local create_cheatsheet = require 'ui.cheatsheet.grid'
			create_cheatsheet(vim.g.nvch_buf, vim.g.nvch_win, 'redraw')
		end,
	})

	vim.keymap.set('n', 'q', function() require('ui.tabufline').close_buffer() end, { buffer = buf })

	vim.keymap.set('n', '<ESC>', function() require('ui.tabufline').close_buffer() end, { buffer = buf })
...
M.rand_hlgroup = function()
	local hlgroups = { 'blue', 'red', 'green', 'yellow', 'orange', 'baby_pink', 'purple', 'white', 'cyan', 'vibrant_green', 'teal' }
