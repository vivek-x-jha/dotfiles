...
M.open = function(buf, win, action)
	...
	for _, val in ipairs(header) do
		...
		if headerw > nvdash_w then nvdash_w = headerw end
	end
	...
	for _, v in ipairs(opts.buttons) do
		...
		if not v.no_gap then table.insert(dashboard, { txt = string.rep(' ', nvdash_w) }) end
	end
	...
	api.nvim_create_autocmd({ 'WinResized', 'VimResized' }, {
		...
		callback = function()
			...
			require('ui.nvdash').open(vim.g.nvdash_buf, vim.g.nvdash_win, 'redraw')
		end,
	})
end
