...
| Pi coding agent | `PI_CODING_AGENT_DIR="$XDG_STATE_HOME/pi/agent"` | Pi auth, sessions, settings, and runtime state stay in XDG state; bootstrap links global instructions and repo-managed Pi resources, including local Ollama models, the SourDiesel TUI theme, extensions, skills, and the `/ship` prompt template. |
