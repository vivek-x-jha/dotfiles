...
- tmux status uses one local `scripts/sys.sh` segment for CPU, RAM, and battery to avoid flicker from staggered plugin shell commands; icons are configurable with `@sys_cpu_icon` and `@sys_ram_icon`.
