...
- `update-tools` supports per-step flags in Bash and Zsh; no flags run the standard workflow without TeX Live, `--all` includes TeX Live, and `--tex` runs only the TeX Live update
