...
- Primary terminal tools: Herdr is installed by the default `herdr` target; eva is installed by the default Rust phase. CIA and eza are legacy migration removals.
...
- Keep Kristy's identity, signing, 1Password vault, Codex preferences, and curated personal Brew profile intact when merging upstream.
